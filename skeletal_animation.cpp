...
// player
glm::vec3 playerPosition = glm::vec3(-1.0f, 1.9f, -8.0f);
...
// enemy
glm::vec3 enemyPosition = glm::vec3(-5.0f, 1.9f, -14.0f);
...
int main()
{
	...
	Model mapModel(FileSystem::getPath("resources/objects/map/dungeon/source/DungeonBlend/DungeonBlend/dungeon_v5.obj"));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case IDLE:
			...
			else if ((glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) || (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS)) {
				blendAmount = 0.0f;
		...
		case IDLE:
			...
			else if ((glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) || playDyingAnim) {
				enemyBlendAmount = 0.0f;
		...
		case IDLE_DYING:
			...
			if (enemyBlendAmount > 0.9f) {
				...
				isEnemyAlive = false;
				//enemyState = DYING_IDLE;
			}
			break;
		case DYING_IDLE:
			if (enemyAnimator.m_CurrentTime > 1.0f) {
				enemyBlendAmount += enemyBlendRate;
				enemyBlendAmount = fmod(enemyBlendAmount, 1.0f);
				enemyAnimator.PlayAnimation(&enemyDyingAnimation, &enemyIdleAnimation, enemyAnimator.m_CurrentTime, enemyAnimator.m_CurrentTime2, enemyBlendAmount);
				if (enemyBlendAmount > 0.9f) {
					enemyBlendAmount = 0.0f;
					float startTime = enemyAnimator.m_CurrentTime2;
					enemyAnimator.PlayAnimation(&enemyIdleAnimation, NULL, startTime, 0.0f, enemyBlendAmount);
